...
function setupSignup() {
  ...
  form.addEventListener("submit", async (event) => {
    ...
    const config = window.ATHLETIC_BIOTECHNICS_COMMERCE || {};
    const endpoint = config.waitlistEndpoint || "";
    ...
    if (endpoint) {
      try {
        ...
        note.textContent = "Could not submit right now. Please try again in a moment.";
        return;
      }
    } else {
      const saved = JSON.parse(localStorage.getItem("ab_waitlist") || "[]");
      saved.push(entry);
      localStorage.setItem("ab_waitlist", JSON.stringify(saved));
    }

    note.textContent = endpoint
      ? "Received. Athletic Biotechnics will follow up with early access details."
      : "Saved locally. Add a waitlist endpoint in commerce-config.js to collect emails online.";
    form.reset();
  });
}
